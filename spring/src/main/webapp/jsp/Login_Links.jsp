...
<script>
	// login 처리 변경 
	var URL_WEB_SERVER			= "<%=SystemMessage.getMessage("URL.WEB.SERVER")%>";
	var BROWSER_TITLE			= "<%=SystemMessage.getMessage("BROWSER.TITLE")%>";
	//alert(URL_WEB_SERVER);
	
	
	var em   = "<%=request.getParameter("em")%>";
	if( em != "Y" && em != "y") {
		window.top.location.replace(URL_WEB_SERVER+"/login/login.do");
	}  
	/*
	//alert(window.location.pathname.split('\/')[1] );
	if( opener == null) {  // 팝업open 되지 않았다.
	    var txt = top.document.title;
	    //alert("1" + txt);
	    if(txt == BROWSER_TITLE) {   // portal login으로 이동 
			if(confirm('비정상 작동 했습니다. 로그인 페이지로 이동하시겠습니까?')) {
				window.top.location.replace(URL_WEB_SERVER+"/login/login.do");
			}
	    }
	    ...
	    if(opener == this) {	// open시칸 주체면
		    var txt = top.document.title;
		    //alert("2" + txt);
		
	    } else {               // 팝업창이면 
			var txt = opener.top.document.title;
		    //alert("3" + txt);
		    if(txt == BROWSER_TITLE) {   // portal login으로 이동 
				if(confirm('비정상 작동 했습니다. 로그인 페이지로 이동하시겠습니까?')) {
					opener.window.top.location.replace(URL_WEB_SERVER+"/login/login.do");
					this.close();
				}
	    	}
	    }
